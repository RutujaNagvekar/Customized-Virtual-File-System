...
void man(char *name)
{
    ...
    {
        printf("Description : Used to remove the data from a file\n");
...
void DisplayHelp()
{
    ...
    printf("stat : To display information from the file using name\n");
    printf("fstat : To display information from the file using descriptor\n");
...
int CreateFile(char *name , int Permission)
{
    ...
    while(temp != NULL)
    {
        if(temp -> FileType == 0)     // to find empty inode
            break;
        temp = temp -> next;
    }
         ...
         else if(count == 3)
         {
             ...
             else if(strcmp(command[0],"open") == 0)
             {
                 ...
                 if(ret == -1)
                     printf("ERROR : Incorrect Parameter\n");
                 if(ret == -2)
                     printf("ERROR : There is no file present\n");
                 if(ret == -3)
                     printf("ERROR : Permission Denied\n");
                 continue;
             }
